...
The class responsible for reading and saving the manga Volume's data from and to the text file "record.txt" is called TextMemory. Through its function read() it creates a new Collection object, obtains the information of each manga volume which is stored in the following text format:
